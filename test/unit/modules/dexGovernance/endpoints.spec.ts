...
import {
    createMethodContext, EventQueue
...
describe('dexGovernance:endpoints', () => {
    ...
    describe('constructor', () => {
        beforeEach(async () => {
            ...
            endpoint = new DexGovernanceEndpoint(dexGovernanceModule.stores, dexGovernanceModule.offchainStores);
        ...
        it('getProposal', async () => {
            ...
            await expect(
                endpoint.getProposal(moduleEndpointContext),
            ).resolves.toHaveProperty("proposal");
        });
        it('getUserVotes', async () => {
            ...
            await expect(
                endpoint.getUserVotes(moduleEndpointContext),
            ).resolves.toMatchObject({ "voteInfos": [{ "amount": BigInt(1000), "decision": 1, "proposalIndex": 0 }] });
        });
        it('getIndexStore', async () => {
            ...
            await expect(
                endpoint.getIndexStore(moduleEndpointContext),
            ).resolves.toMatchObject({ indexStore: index });
        });
    });
});
