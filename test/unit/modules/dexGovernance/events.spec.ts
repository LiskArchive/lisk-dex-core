...
describe('DexGovernanceModule:events', () => {
	...
	it('events should be registered and inherit from BaseEvent', () => {
		expect(dexGovernanceModule.events.get(GeneratorRewardMintedEvent)).toBeInstanceOf(BaseEvent);
		expect(dexGovernanceModule.events.get(ValidatorTradeRewardsPayoutEvent)).toBeInstanceOf(
			BaseEvent,
		);
	});
});
