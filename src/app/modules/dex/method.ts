...
export class DexMethod extends BaseMethod {
    public async poolExists(methodContext: ImmutableMethodContext, poolsStore: PoolsStore, poolId: PoolID): Promise<boolean> {
    ...
    public async updateIncentivizedPools(
        ...
        const dexGlobalStoreData: DexGlobalStoreData = await getDexGlobalData(methodContext, this.stores);

        for (const incentivizedPool of dexGlobalStoreData.incentivizedPools) {
            await updatePoolIncentives(methodContext, this.stores, incentivizedPool.poolId, currentHeight);
        }
