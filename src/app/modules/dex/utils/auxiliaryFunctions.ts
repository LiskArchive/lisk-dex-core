...
import {
    NamedRegistry
} from 'lisk-framework/dist-node/modules/named_registry';
...
export const transferToPool = async (tokenMethod: TokenMethod, methodContext, senderAddress: Address, poolId: PoolID, tokenId: TokenID, amount: bigint): Promise < void > => {
...
export const transferPoolToPool = async (
    ...
    await tokenMethod.unlock(methodContext, poolAddressSend, MODULE_ID_DEX.toString(), tokenId, amount)
    await tokenMethod.transfer(methodContext, poolAddressSend, poolAddressReceive, tokenId, amount)
    await tokenMethod.lock(methodContext, poolAddressReceive, MODULE_ID_DEX.toString(), tokenId, amount)
}
...
export const transferToProtocolFeeAccount = async (
    ...
    const {
        protocolFeeAddress
    } = await settings.get(methodContext, Buffer.from([]));
    await tokenMethod.transfer(methodContext, senderAddress, protocolFeeAddress, tokenId, amount)
}

export const checkPositionExistenceAndOwnership = async (stores: NamedRegistry, events: NamedRegistry, methodContext, senderAddress: Address, positionID: PositionID): Promise < void > => {
...
export const collectFeesAndIncentives = async (events: NamedRegistry, stores: NamedRegistry, tokenMethod, methodContext, positionID: PositionID): Promise < void > => {
    ...
    const [collectedFees0, collectedFees1, feeGrowthInside0, feeGrowthInside1] = await computeCollectableFees(stores, methodContext, positionID)

    if (collectedFees0 > 0) {
        await transferFromPool(tokenMethod, methodContext, poolID, ownerAddress, getToken0Id(poolID), collectedFees0)
    }
    if (collectedFees1 > 0) {
        await transferFromPool(tokenMethod, methodContext, poolID, ownerAddress, getToken1Id(poolID), collectedFees1)
    }
    ...
    const [collectableFeesLSK, incentivesForPosition] = await computeCollectableIncentives(dexGlobalStore, TokenMethod, positionID, collectedFees0, collectedFees1)

    await tokenMethod.transfer(ADDRESS_LIQUIDITY_PROVIDERS_REWARDS_POOL, ownerAddress, TOKEN_ID_REWARDS, incentivesForPosition)
...
export const computeCollectableFees = async (stores: NamedRegistry, methodContext: MethodContext, positionID: PositionID): Promise < [bigint, bigint, Q96, Q96] > => {
    const positionsStore = stores.get(PositionsStore);
    const positionInfo = await positionsStore.get(methodContext, positionID)
    const [feeGrowthInside0, feeGrowthInside1] = await getFeeGrowthInside(stores, methodContext, positionID)

    const collectableFees0 = roundDownQ96(mulQ96(subQ96(feeGrowthInside0, bytesToQ96(positionInfo.feeGrowthInsideLast0)), positionInfo.liquidity))
    const collectableFees1 = roundDownQ96(mulQ96(subQ96(feeGrowthInside1, bytesToQ96(positionInfo.feeGrowthInsideLast1)), positionInfo.liquidity))

    return [collectableFees0, collectableFees1, feeGrowthInside0, feeGrowthInside1]
}


export const computeCollectableIncentives = async (dexGlobalStore, tokenMethod, positionID: PositionID, collectableFees0: bigint, collectableFees1: bigint): Promise < [bigint, bigint] > => {
    const poolID = getPoolIDFromPositionID(positionID)
    ...
    const availableLPIncentives = await tokenMethod.getAvailableBalance(ADDRESS_LIQUIDITY_PROVIDERS_REWARDS_POOL, TOKEN_ID_REWARDS)
...
export const createPool = async (settings, methodContext, poolsStore: PoolsStore, tokenID0: TokenID, tokenID1: TokenID, feeTier: number, initialSqrtPrice: Q96): Promise < number > => {
...
export const createPosition = async (methodContext: MethodContext, stores: NamedRegistry, senderAddress: Address, poolID: PoolID, tickLower: number, tickUpper: number): Promise < [number, PositionID] > => {
    const dexGlobalStore = stores.get(DexGlobalStore);
...
export const getFeeGrowthInside = async (stores: NamedRegistry, methodContext: MethodContext, positionID: PositionID): Promise < [bigint, bigint] > => {
    const poolsStore = stores.get(PoolsStore);
    const positionsStore = stores.get(PositionsStore);
    const priceTicksStore = stores.get(PriceTicksStore);
    const positionInfo = await positionsStore.get(methodContext, positionID)
    ...
    const lowerTickInfo = await priceTicksStore.getKey(methodContext, [poolID, tickToBytes(tickLower)])
    const upperTickInfo = await priceTicksStore.getKey(methodContext, [poolID, tickToBytes(tickUpper)])
    ...
    if (tickCurrent >= tickLower) {
        ...
        feeGrowthBelow0 = subQ96(bytesToQ96(poolInfo.feeGrowthGlobal0), bytesToQ96(lowerTickInfo.feeGrowthOutside0))
        feeGrowthBelow1 = subQ96(bytesToQ96(poolInfo.feeGrowthGlobal1), bytesToQ96(lowerTickInfo.feeGrowthOutside1))
    }
    ...
    if (tickCurrent < tickUpper) {
        ...
        feeGrowthAbove0 = subQ96(bytesToQ96(poolInfo.feeGrowthGlobal0), bytesToQ96(upperTickInfo.feeGrowthOutside0))
        feeGrowthAbove1 = subQ96(bytesToQ96(poolInfo.feeGrowthGlobal1), bytesToQ96(upperTickInfo.feeGrowthOutside1))
    }

    const feeGrowthInside0 = subQ96(subQ96(bytesToQ96(poolInfo.feeGrowthGlobal0), feeGrowthBelow0), feeGrowthAbove0)
    const feeGrowthInside1 = subQ96(subQ96(bytesToQ96(poolInfo.feeGrowthGlobal1), feeGrowthBelow1), feeGrowthAbove1)
...
export const getPoolIDFromPositionID = (positionID: PositionID): Buffer => positionID.slice(-NUM_BYTES_POOL_ID)


export const updatePosition = async (methodContext: MethodContext, events: NamedRegistry, stores: NamedRegistry, tokenMethod, positionID: PositionID, liquidityDelta: bigint): Promise < [bigint, bigint] > => {
    const poolsStore = stores.get(PoolsStore);
    const positionsStore = stores.get(PositionsStore);
    const priceTicksStore = stores.get(PriceTicksStore);
    const positionInfo = await positionsStore.get(methodContext, positionID);
    ...
    const poolID = getPoolIDFromPositionID(positionID)
    const poolInfo = await poolsStore.get(methodContext, poolID)
    const lowerTickInfo = await priceTicksStore.getKey(methodContext, [poolID, q96ToBytes(tickToPrice(positionInfo.tickLower))])
    const upperTickInfo = await priceTicksStore.getKey(methodContext, [poolID, q96ToBytes(tickToPrice(positionInfo.tickUpper))])
    ...
    const ownerAddress = await getOwnerAddressOfPosition(positionsStore, positionID)
    if (liquidityDelta > 0) {
        await transferToPool(tokenMethod, methodContext, ownerAddress, poolID, getToken0Id(poolID), amount0)
        await transferToPool(tokenMethod, methodContext, ownerAddress, poolID, getToken1Id(poolID), amount1)
    } else {
        await transferFromPool(tokenMethod, methodContext, poolID, ownerAddress, getToken0Id(poolID), amount0)
        await transferFromPool(tokenMethod, methodContext, poolID, ownerAddress, getToken1Id(poolID), amount1)
    }
    ...
    if (lowerTickInfo.liquidityGross === BigInt(0)) {
        await priceTicksStore.delKey(methodContext, [poolID, q96ToBytes(tickToPrice(positionInfo.tickLower))])
    } else {
        await priceTicksStore.setKey(methodContext, [poolID, q96ToBytes(tickToPrice(positionInfo.tickLower))], lowerTickInfo)
    }

    if (upperTickInfo.liquidityGross === BigInt(0)) {
        await priceTicksStore.delKey(methodContext, [poolID, q96ToBytes(tickToPrice(positionInfo.tickUpper))])
    } else {
        await priceTicksStore.setKey(methodContext, [poolID, q96ToBytes(tickToPrice(positionInfo.tickUpper))], upperTickInfo)
    }
