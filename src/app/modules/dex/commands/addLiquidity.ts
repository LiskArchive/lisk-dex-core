...
import {
    addLiquidityParamsSchema
} from '../schemas';
import {
    PoolsStore,
    PositionsStore} from '../stores';
import {
    AddLiquidityParamsData,
} from '../types';
import {
...
export class AddLiquidityCommand extends BaseCommand {
    ...
    public schema = addLiquidityParamsSchema;
    ...
    public async verify(ctx: CommandVerifyContext < AddLiquidityParamsData > ): Promise < VerificationResult > {
        try {
            validator.validate(addLiquidityParamsSchema, ctx.params);
        } catch (err) {
            return {
    ...
    public async execute(ctx: CommandExecuteContext < AddLiquidityParamsData > ): Promise < void > {
        ...
        const liquidity = getLiquidityForAmounts(q96ToInt(bytesToQ96(currentSqrtPrice)),
            tickLowerSqrtPrice,
            tickUpperSqrtPrice,
            amount0Desired,
            amount1Desired);
        ...
        this.events.get(PositionUpdatedEvent).add(methodContext, {
            senderAddress,
            positionID,
            amount0,
            tokenID0,
